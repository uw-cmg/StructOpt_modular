#+TITLE:     Tutorial for submitting jobs using the JobManager
#+AUTHOR:    Zhongnan Xu
...
* Configuration \label{sec-config}
...
* Submitting jobs \label{sec-submit}
** Single job \label{sec-submit-single}
...
** Multiple jobs \label{sec-submit-multiple}
...
:END:
* Restarting jobs \label{sec-restart]

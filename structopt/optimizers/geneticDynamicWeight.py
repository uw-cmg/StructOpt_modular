...
class GeneticAlgorithm(object):
    ...
    def step(self):
        ...
        if gparameters.generation % 20 == 0 and gparameters.mpi.rank == 0:  # every 20 steps update the weight
            chi2, energy = 0, 0
            cnt = 0
            for individual in self.population:
                if individual.fits['FEMSIM'] < 1 and -4 < individual.fits['LAMMPS'] < 0:   
                    chi2 += individual.fits['FEMSIM']
                    energy += individual.fits['LAMMPS']
                    cnt += 1
            if cnt > 0:
                gparameters.fitnesses.FEMSIM.weight = abs(energy) / chi2
                print('FEMSIM weight (alpha) updated to be {}'.format(gparameters.fitnesses.FEMSIM.weight))
